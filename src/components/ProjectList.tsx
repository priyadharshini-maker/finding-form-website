    ...
    <div style={{ height: '230vh' }}>
      <div className="sticky top-0 w-full h-screen overflow-hidden bg-bg pt-[8px]">
        <motion.div
          ...
          className="mx-[10px] w-full h-full overflow-hidden shadow-2xl relative group cursor-pointer origin-top"
        >
          <Link to={`/project/${project.id}`} className="block w-full h-full">
            ...
            {/* Hover overlay with full details and CTA */}
            <div className="absolute inset-0 bg-black/45 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center gap-4 px-8">
              <div className="text-center transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500 flex flex-col items-center gap-3">
                ...
                <h3 className="text-bg font-medium tracking-widest uppercase text-xl md:text-3xl">
                  {project.title}
                </h3>
                <p className="text-bg/80 font-serif italic text-base md:text-xl">
                  {project.category}
                </p>
                {project.description && (
                  <p className="text-bg/70 text-sm md:text-base max-w-xl text-center leading-relaxed mt-1">
                    {project.description}
                  </p>
                )}
